...
namespace AIOverhaulPatcher.Utilities
{
    public static class ExLoadOrderUtilities
    {
        ...
        public static List<TMajor> GetOverrides<TMajor>(this TMajor record, SynthesisState<ISkyrimMod, ISkyrimModGetter> state) where TMajor : class, IMajorRecordCommonGetter
        {
            ...
            return state.LoadOrder.PriorityOrder.Skip(order+1).Select(x => x.Mod).SelectMany(x=>x?.GetTopLevelGroupGetter<TMajor>()).Where(x => x.Value.FormKey.ID == record.FormKey.ID).Select(x => x.Value).ToList();
        }
        ...
        {
            var order = record.GetOrder(state);
            return state.LoadOrder.PriorityOrder.Take(order).Select(x => x.Mod).SelectMany(x => x?.GetTopLevelGroupGetter<TMajor>()).Where(x => x.Value.FormKey.ID == record.FormKey.ID).Select(x => x.Value).ToList();
        }
        ...
        {
            return state.LoadOrder.PriorityOrder.Select(x => x.Mod).SelectMany(x=>x?.GetTopLevelGroupGetter<TMajor>()).Where(x => x.Value.FormKey.ID == record.FormKey.ID).Select(x => x.Value).First();
        }
